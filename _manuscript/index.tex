...
In an important contribution, \citet{chanda2020} leveraged nighttime
light data to document evidence of regional convergence across 520
districts in India between 1996 and 2010. Their analysis showed that
...
\citet{chanda2020} in three key methodological directions. First, we
develop an interactive web-based visualization tool that allows
researchers to dynamically explore spatial and temporal patterns in the
nighttime light data. This tool facilitates the identification of
...
convergence equations, highlighting that spatial autocorrelation is an
inherent feature of satellite data and the regional convergence process.
Third, we employ a spatial Durbin model that explicitly accounts for
spatial spillovers, quantifying how neighborhood effects influence the
...
\section{Data, model specification, and
...
pioneered by \citet{henderson2012}, has utilized satellite nighttime
light data as a proxy for economic activity at sub-national levels.
...
countries. For instance, \citet{adhikari.dhitalEI2021} examine the
impact of decentralization on regional convergence using nightlight
data, while \citet{pinkovskiy2016newer} argue that nightlight data
better captures aggregate economic activity compared to household
surveys. Similarly, \citet{lessmann2017} leverage nightlight data to
estimate GDP per capita and income inequality globally, and
\citet{gennaioli2014growth} study regional convergence through the lens
of light data.

Nightlight data has been widely utilized in India across various
contexts. For example, \citet{asher2016market} and
\citet{cook.shahTRoEaS2022} use nightlight data to analyze the impact of
road infrastructure and public welfare programs, respectively.
Similarly, \citet{jha.talathiEDaCC2023} examine the effects of
colonization, while \citet{chanda.cookJoM2022} investigate the impact of
demonetization using nightlight data. \citet{beyer.etalWD2021} employ it
to study the effects of COVID-19. In the context of convergence studies,
\citet{chakravarty2017will} finds state-level divergence in India using
nightlight data, while \citep{chanda2020} highlights district-level
convergence using the same.

Our study draws inspiration from \citep{chanda2020}. Following their
approach, we use the per capita growth in nightlights as the dependent
variable and the initial nightlights per capita as the primary variable
of interest. These variables are derived from nightlight data released
...
data.\footnote{Following \citet{chanda2020}, our sample consists of 520
  districts (out of a possible 593). There were 593 districts in 2001,
  which rose to 640 districts in the 2011 census. To match the districts
  among two census files, we summed up the curved-out districts with
  their origin districts in the 2011 census files. Eight districts among
  47 new districts were created by curving out some areas from multiple
...
listed in Table A.1 in \citet{chanda2020}.

\subsection{Extensions: Interactive visualizations, spatial dependence
testing, and spillover
modeling}\label{extensions-interactive-visualizations-spatial-dependence-testing-and-spillover-modeling}

\subsubsection{Interactive spatial
...
spatial econometric techniques. In particular, \citet{ertur2007growth}
and \citet{fischer2011spatial} argue that the spatial Durbin model can
properly account for spatial spillovers in the convergence process. This
...
\subsubsection{Spatial spillover
modeling}\label{spatial-spillover-modeling}

Our spatial spillover modeling builds upon the spatial Solow growth
model developed by \citet{ertur2007growth} and
\citet{fischer2011spatial}, which extends the traditional Solow
...
Based on this theoretical framework, \citet{ertur2007growth} derived a
...
\section{Results: Replication and
...
of \citet{chanda2020}, we developed an interactive web-based
...
36\% larger than conventional non-spatial estimates of
\citet{chanda2020}. This finding implies that non-spatial convergence
models may considerably underestimate the speed of regional convergence.
