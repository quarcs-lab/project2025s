...
Quarto uses intermediate cache files and generates output files for notebook previews:

**Cache (intermediates — safe to delete, regenerated by render):**

1. **`_freeze/`** — execution results (figures + JSON)
2. **`.quarto/embed/`** — internal embed cache (hidden directory)
3. **`notebooks/*.out.ipynb`** — executed notebook intermediates (stale copies cause label/content mismatches)

**Output (must be committed for GitHub Pages — readers click on these):**

4. **`notebooks/*-preview.html`** — rendered notebook pages linked from HTML sidebar
5. **`notebooks/*.embed-preview.html`** — embedded notebook preview pages

`freeze: auto` only handles `_freeze/` automatically. It does **NOT** invalidate the other layers — this causes stale previews and missing figure labels even after notebook source changes. To ensure everything is rebuilt, **always use after notebook edits:**

```bash
bash scripts/clean-render.sh
```

This clears all caches, runs a full manuscript render (which regenerates notebook preview pages), then fixes the REGION PDF with 4 LaTeX passes. For manuscript-only edits (changes to `index.qmd` text), plain `quarto render index.qmd` is sufficient.

**Important:** Notebook preview pages (`*-preview.html`) are only generated during a full `quarto render` (no `--to` flags). Using `quarto render --to html` generates only the main article HTML, not the notebook previews. This is why `clean-render.sh` uses a full render as its first step.
